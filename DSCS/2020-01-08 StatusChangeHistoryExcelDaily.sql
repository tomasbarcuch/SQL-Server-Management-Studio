declare @from as DATE = '2019-12-06'
declare @to as DATE = '2019-12-07'
...
when 'LpEntpacktReal' then '560'
when 'LpVerladen' then '700'
when 'LpAusgeliefert' then '410'
when 'LpLackiertFrei' then '210'
when 'HuVorverpackt' then '315'
when 'HuVerpacktausVorverpackt' then  '320'
when 'LpVirtuellVerpackt' then '305'
...
when 'LpEntpacktReal' then 'Entpacken real abgeschlossen'
...
when 'HuVorverpackt' then 'Packdaten berechnet'
when 'HuVerpacktausVorverpackt' then 'Maschinenboden abgeschlossen'
when 'LpVirtuellVerpackt' then 'Packdaten unverpackte BG'
...
inner join status S on WFE.StatusId = S.id
  ...
  left join (select loosepartid, HandlingUnitId from WarehouseEntry with (NOLOCK)
...
--and WFE.Updated between cast(cast(getdate() as date) as datetime) and cast(cast(getdate()+1 as date) as datetime)
and WFE.Updated between cast(cast(@from as date) as datetime) and cast(cast(@to as date) as datetime)
...
where main.Datum 
--between cast(cast(getdate() as date) as datetime) and cast(cast(getdate()+1 as date) as datetime)

 between cast(cast(@from as date) as datetime) and cast(cast(@to as date) as datetime)
...
inner join CustomField CF on CV.CustomFieldId = CF.Id
where CV.CustomFieldId in 
(select id from CustomField with (NOLOCK) where name in ('DatumStart','DatumEnd'))) dates on lockheader.EntityID = dates.EntityId
...
--and [Kisten Nr.] in ('0001500038080003') or [PackstückIdent (Beipack)] = '0000038080/000201'