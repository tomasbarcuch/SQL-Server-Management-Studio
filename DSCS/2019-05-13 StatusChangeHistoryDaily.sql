...
group by berlin.EntityId,LP.code) RetDel on data.LPid = RetDel.entityId and RetDel.Rücklieferdatum = DATA.CreatedWFE) MAIN
where main.Datum between cast(cast(getdate() as date) as datetime) and cast(cast(getdate()+1 as date) as datetime)
