...
,cast (isnull(vv_lp.ABMESS1_MM,0)/10 as varchar)+'-'+cast(isnull(vv_lp.ABMESS2_MM,0)/10 as varchar)+'-'+cast(isnull(vv_lp.ABMESS3_MM,0)/10 as Varchar)/*+'-'+cast(vv_lp.ABMESS4_MM as Varchar)+'-'+cast(vv_lp.ABMESS5_MM as Varchar)+'-'+cast(vv_lp.ABMESS6_MM as Varchar)*/
 as 'LBH'
,S.TXX as 'Status'
, case O.TXX
...
and (((case when @packed = 0  and (vv_lp.Z_KISTEN = '' or vv_lp.Z_KISTEN is null) then 1 else 0 end)=1)
or 
((case when @packed = 1 then 1 else 0 end)=1))


and vv_lp.zaehler not in (select content from 
CustomValue CV 
...
and (((case when @packed = 0  and dscs_lp.TopHandlingUnitId is null then 1 else 0 end)=1)
or 
((case when @packed = 1 then 1 else 0 end)=1))
...
,'' as 'SH.TruckNumber'
,'' as 'SH.Adress to'
,vv_hu.LAGERORT1+vv_hu.LAGERORT2+vv_hu.LAGERORT3 as 'HU.Actual bin'
...
where vv_hu.W_Standort not in (0)
and vv_hu.zaehler not in (select content from 
CustomValue CV 
inner join CustomField CF on CV.CustomFieldId = CF.id and cf.ClientBusinessUnitId = (Select id from BusinessUnit where name = 'KRONES GLOBAL')
where cf.name = 'VVID' and len(content)>0)
...
,CUS.W_ORT_VP as 'HU.PackingLocation'
,'' as 'CF_Material Nr.'
...
left join CustomField CF on CV.CustomFieldId = CF.id
where cf.name = 'VVID'  --and S.Name in ('BOX_CLOSED')
) DATA
