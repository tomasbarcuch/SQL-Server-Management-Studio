...
,cast(case Orders.DateTimeOfPlannedDeliveryToCustomer when 'NOW' then Getdate() else Orders.DateTimeOfPlannedDeliveryToCustomer end as date) 'späteste Sendung'
,NULL 'Text 1'
...
,ActZone.Name as Zone
 ...
 inner join (select min(DATA.created) Created, EntityId,StatusId  from (
select 
min(WFE.created)  over (PARTITION by WFE.EntityId,WFE.StatusID) as Created, 
WFE.EntityId,
WFE.StatusId
from WorkflowEntry WFE 
inner join BusinessUnitPermission BUP on WFE.StatusId = BUP.StatusId and BUP.BusinessUnitId = (Select id from BusinessUnit where name = 'KRONES GLOBAL')
) DATA group by EntityId,StatusId) WFE on dscs_hu.StatusId = WFE.StatusId and WFE.EntityId = dscs_hu.id
 left join (
select min(data.created) Created,Data.EntityId from( select
min(WFE.created)  over (PARTITION by WFE.EntityId) as Created, 
WFE.EntityId
from WorkflowEntry WFE 
where WFE.StatusId = '7d8927fb-456f-48bb-a44b-6e6dc555ddda'
)DATA group by EntityId) PACKED on PACKED.EntityId = dscs_hu.id--BOX_CLOSED

 left join (select min(data.created) Created,Data.EntityId from( 
select 
min(WFE.created)  over (PARTITION by WFE.EntityId) as Created, 
WFE.EntityId
from WorkflowEntry WFE 
where WFE.StatusId in ('ad6082e9-3f57-40f8-adc6-504b5630b113','45a45541-730d-42bf-8ae5-c0cff45746a3')
) DATA group by EntityId) INB on INB.EntityId = dscs_hu.id  --IN_STOCK, STORED
...
left join Translation TypeDE on [Type].id = TypeDE.EntityId and TypeDE.[Language] = 'de' and TypeDE.[Column] = 'Code'
 ...
 left join Zone ActZone on ActBin.ZoneId = ActZone.Id
 ...
 left join ShipmentHeader SH on dscs_hu.ShipmentHeaderId = SH.id and SH.[Type] = 0
