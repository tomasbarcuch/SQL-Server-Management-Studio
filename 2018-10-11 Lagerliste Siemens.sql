...
cast(min(CV_WE.Content) as date) as 'Datum WE (fix)',
L.Code+'/'+Z.Name+'/'+B.Code as 'Lagerort'

from loosepart LP
inner join BusinessUnitPermission BUP on LP.id = BUP.LoosePartId
INNER join customfield CF_WE on BUP.BusinessUnitId = CF_WE.clientbusinessunitid and CF_WE.name = 'Datum WE (fix)'
INNER join customvalue CV_WE on CV_WE.customfieldid = CF_WE.Id and CV_WE.EntityId = LP.Id
INNER join customfield CF_POSlp on BUP.BusinessUnitId = CF_POSlp.clientbusinessunitid and CF_POSlp.name = 'Position'
INNER join customvalue CV_POSlp on CV_POSlp.customfieldid = CF_POSlp.Id and CV_POSlp.EntityId = LP.Id
INNER join customfield CF_FNlp on BUP.BusinessUnitId = CF_FNlp.clientbusinessunitid and CF_FNlp.name = 'Feld Nr.'
...
where lp.ActualHandlingUnitId is null and BUP.BusinessUnitId =  'f1a46d31-c25a-4459-95d8-8db804cb7bf2' --$P{BUid}    
and s.name not in ('lpNew')
and L.Code+'/'+Z.Name+'/'+B.Code is not null
...
INNER join customfield CF_WE on BUP.BusinessUnitId = CF_WE.clientbusinessunitid and CF_WE.name = 'Datum Fertigung'
INNER join customvalue CV_WE on CV_WE.customfieldid = CF_WE.Id and CV_WE.EntityId = HU.Id
INNER join customfield CF_POShu on BUP.BusinessUnitId = CF_POShu.clientbusinessunitid and CF_POShu.name = 'Position'
INNER join customvalue CV_POShu on CV_POShu.customfieldid = CF_POShu.Id and CV_POShu.EntityId = HU.Id
INNER join customfield CF_FNhu on BUP.BusinessUnitId = CF_FNhu.clientbusinessunitid and CF_FNhu.name = 'Feld Nr.'
...
where HU.Empty = 0 and BUP.BusinessUnitId = 'f1a46d31-c25a-4459-95d8-8db804cb7bf2' --$P{BUid}  
and s.name not in ('huNew')
and L.Code+'/'+Z.Name+'/'+B.Code is not null
...
left join BusinessUnit BU on DATA.BusinessUnitId = BU.Id
