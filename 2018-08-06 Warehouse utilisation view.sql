...
cast(case when max(OnWarehouse.LocCapacity) > 0 then  Sum(Onwarehouse.basearea)/max(OnWarehouse.LocCapacity)*100 else 100 end as int) Utilisation,
Calender.dates date, 
...
left outer join (select L.ClientBusinessUnitId, L.LocationId,sum(cast(L.Length as decimal)*cast(L.Width as decimal)/1000000) as LocCapacity  from bin L where  L.[Disabled] = 0 group by L.ClientBusinessUnitId,L.LocationId) LCap on data.ClientBusinessUnitId = LCap.ClientBusinessUnitId and data.LocationId = LCap.LocationId
...
where calender.DATES between OnWarehouse.Inbounddate and OnWarehouse.Outbounddate
--and OnWarehouse.ClientBusinessUnitId = 'ad5a5f6b-8ec4-463b-836a-0a8cb60a39d2'
