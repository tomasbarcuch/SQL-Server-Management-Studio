...
and case when ( 
((LP.TopHandlingUnitId is null
--and dim.dvContent like $P{ProjectNr} 
and LP.TopHandlingUnitId is null
and  (s.name in ('lpPacked','LpPackedManually'))))
or ( 
LP.id in (
    select LoosePartId from PackingRule PR
    inner join HandlingUnit HU on PR.ParentHandlingUnitId = HU.Id
...
) then 1 else 0 end in (1,1) --($P{JustPacked},1)*/
...
where cast(Datum as date) >= '2019-08-30'--$P{FromDate}
and cast(Datum as date) <=  '2019-08-30'--$P{ToDate}
